...
// compactNode convert a normal trie node to a a node that only contains the raw data
func compactNode(n node) node {
	switch n := n.(type) {
	case *shortNode:
		sn := &shortNode{Key: hexToCompact(n.Key)}
		if hash, _ := n.Val.cache(); hash != nil {
			sn.Val = hash
		} else {
			sn.Val = n.Val
		}
		return simplifyNode(sn)

	case *fullNode:
		fn := &fullNode{}
		for i := 0; i < len(n.Children); i++ {
			if n.Children[i] != nil {
				if val, _ := n.Children[i].cache(); val != nil {
					fn.Children[i] = val
				} else {
					fn.Children[i] = n.Children[i]
				}
			}
		}
		return simplifyNode(fn)
...
func (db *Database) WaitAsyncCommit() {
	db.flushLatch.Wait()
}
