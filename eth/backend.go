...
import (
	...
	"github.com/QEasyWeb3/QEasyChain/consensus/clique"
	"github.com/QEasyWeb3/QEasyChain/consensus/democracy"
	"github.com/QEasyWeb3/QEasyChain/core"
	"github.com/QEasyWeb3/QEasyChain/core/bloombits"
	"github.com/QEasyWeb3/QEasyChain/core/rawdb"
	"github.com/QEasyWeb3/QEasyChain/core/state/pruner"
	"github.com/QEasyWeb3/QEasyChain/core/types"
	"github.com/QEasyWeb3/QEasyChain/core/vm"
	"github.com/QEasyWeb3/QEasyChain/eth/downloader"
	"github.com/QEasyWeb3/QEasyChain/eth/ethconfig"
	"github.com/QEasyWeb3/QEasyChain/eth/filters"
	"github.com/QEasyWeb3/QEasyChain/eth/gasprice"
...
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	// do some extra work if consensus engine is democracy.
	if democracyEngine, ok := eth.engine.(*democracy.Democracy); ok {
		// set chain & state fn
		democracyEngine.SetChain(eth.blockchain)
		democracyEngine.SetStateFn(eth.blockchain.StateAt)

		// set consensus-related transaction validator
		eth.txPool.InitTxFilter(democracyEngine)
	}
...
func (s *Ethereum) shouldPreserve(block *types.Block) bool {
	...
	if _, ok := s.engine.(*democracy.Democracy); ok {
		return false
	}
...
func (s *Ethereum) StartMining(threads int) error {
	...
	if !s.IsMining() {
		...
		if democracy, ok := s.engine.(*democracy.Democracy); ok {
			wallet, err := s.accountManager.Find(accounts.Account{Address: eb})
			if wallet == nil || err != nil {
				log.Error("Etherbase account unavailable locally", "err", err)
				return fmt.Errorf("signer missing: %v", err)
			}
			democracy.Authorize(eb, wallet.SignData, wallet.SignTx)
		}
...
func (s *Ethereum) StartAttestation() {
	if s.IsMining() {
		if c, ok := s.engine.(*democracy.Democracy); ok {
