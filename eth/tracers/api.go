			...
			for task := range tasks {
				...
				for i, tx := range task.block.Transactions() {
					...
					if isDoubleSignPunishTx {
						res, err = api.traceApplyDoubleSignPunishTx(ctx, msg.From(), tx, txctx, blockCtx, task.statedb, config)
...
// TODO Reuse code
func (api *API) traceApplyDoubleSignPunishTx(ctx context.Context, sender common.Address, tx *types.Transaction, txctx *Context, vmctx vm.BlockContext, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	// Assemble the structured logger or the JavaScript tracer
	var (
		tracer vm.EVMLogger
		err    error
