...
import (
	...
	"github.com/QEasyWeb3/QEasyChain/accounts"
	...
	"github.com/QEasyWeb3/QEasyChain/consensus/misc"
	...
	"github.com/QEasyWeb3/QEasyChain/ethdb"
	...
	"github.com/QEasyWeb3/QEasyChain/trie"
...
// Democracy is the proof-of-stake-authority consensus engine proposed to support the
// Ethereum testnet following the Ropsten attacks.
type Democracy struct {
	chainConfig *params.ChainConfig     // ChainConfig to execute evm
	config      *params.DemocracyConfig // Consensus engine configuration parameters
...
// New creates a Democracy proof-of-stake-authority consensus engine with the initial
...
func New(chainConfig *params.ChainConfig, db ethdb.Database) *Democracy {
	...
	if (conf.SysContractAdmin != common.Address{}) {
		system.AdminForDevelopChain = conf.SysContractAdmin
	}
...
func (c *Democracy) GetDb() ethdb.Database {
	return c.db
}

func (c *Democracy) SetChain(chain consensus.ChainHeaderReader) {
	c.chain = chain
}

// SetStateFn sets the function to get state.
func (c *Democracy) SetStateFn(fn StateFn) {
	c.stateFn = fn
}

// Author implements consensus.Engine, returning the Ethereum address recovered
// from the signature in the header's extra-data section.
func (c *Democracy) Author(header *types.Header) (common.Address, error) {
	return header.Coinbase, nil
	// return ecrecover(header, c.signatures)
}

// VerifyHeader checks whether a header conforms to the consensus rules.
func (c *Democracy) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header, seal bool) error {
...
func (c *Democracy) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
...
func (c *Democracy) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Democracy) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Democracy) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
...
func (c *Democracy) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
func (c *Democracy) VerifySeal(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// from.
func (c *Democracy) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Democracy) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// rewards given.
func (c *Democracy) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB,
...
func (c *Democracy) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB,
...
func (c *Democracy) prepareFinalize(chain consensus.ChainHeaderReader, header *types.Header,
...
func (c *Democracy) updateValidators(vmCtx *systemcontract.CallContext, chain consensus.ChainHeaderReader, mined bool) error {
	header := vmCtx.Header
...
func (c *Democracy) tryDistributeBlockFee(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
...
func (c *Democracy) tryLazyPunish(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	number := header.Number.Uint64()
	snap, err := c.snapshot(chain, number-1, header.ParentHash, nil)
	if err != nil {
		return err
...
func (c *Democracy) getTopValidators(chain consensus.ChainHeaderReader, header *types.Header) ([]common.Address, error) {
...
func (c *Democracy) Authorize(validator common.Address, signFn ValidatorFn, signTxFn SignTxFn) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *Democracy) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
...
func (c *Democracy) SealHash(header *types.Header) common.Hash {
	return SealHash(header)
}

// Close implements consensus.Engine. It's a noop for chaos as there are no background threads.
func (c *Democracy) Close() error {
	return nil
}

// APIs implements consensus.Engine, returning the user facing RPC API to allow
// controlling the validator voting.
func (c *Democracy) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	return []rpc.API{{
		Namespace: "democracy",
		Version:   "1.0",
		Service:   &API{chain: chain, democracy: c},
		Public:    false,
	}}
}
...
// DemocracyRLP returns the rlp bytes which needs to be signed for the proof-of-stake-authority
...
func DemocracyRLP(header *types.Header) []byte {
	b := new(bytes.Buffer)
...
func (c *Democracy) ExtraValidateOfTx(sender common.Address, tx *types.Transaction, header *types.Header) error {
